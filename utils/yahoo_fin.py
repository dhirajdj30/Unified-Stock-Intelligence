...
# """
#     MSFT: Period '1day' is invalid, must be one of: 1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max
...
# info = msft.info
# print(info['symbol'], info['regularMarketPrice'])
...
class ChatRequest(BaseModel):
    message: str
    provider: str = "gemini"   # default to Gemini, but user can choose
...
def chat(data: ChatRequest):
    logging.info(f"POST /api/chat called with message.")

    if not data.message:
        raise HTTPException(status_code=400, detail="Message is required")

    try:
        # Dynamically load the model via your factory
        llm = get_llm(
        ...
        response_text = llm.invoke(data.message)
...
if __name__ == "__main__":
    uvicorn.run("utils.yahoo_fin:app", host="0.0.0.0", port=8000, reload=True)
